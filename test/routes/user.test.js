...
test('GET user/all', async () => {
  const myMock = jest.fn().mockReturnValue(usersMock);
  db.User.findAll = myMock;
  await request(app)
    .get('/api/v1/user/all')
...
test('POST user/register', async () => {
  ...
  db.User.create = jest.fn().mockReturnValue(mockModel);
  await request(app)
...
test('POST user/register - Existing Email', async (done) => {
  db.User.findAll = jest.fn().mockReturnValue(usersMock);
  await request(app)
    .post('/api/v1/user/register')
    .set('Authorization', `Bearer ${getToken()}`)
    .send({
    ...
    .expect(400)
    .then((response) => {
      expect(response.body.message).toEqual('Existing User\'s email');
      done();
    });
});
...
test('DELETE user/delete - Delete User', async () => {
  db.User.destroy = jest.fn().mockReturnValue([1]);
  await request(app)
